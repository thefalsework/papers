...
A paper-by-paper overview with abstracts, current open validation items, and cross-reference graph lives at [`papers/INDEX.md`](papers/INDEX.md). The canonical statement of the refined four-cells-plus-gate architecture lives at [`papers/comma-formal-structure-note.md`](papers/comma-formal-structure-note.md), with the version-tracked validation status at [`validation/claims/five-position-derivation-formalization.md`](validation/claims/five-position-derivation-formalization.md) (currently v0.6). The single named upstream blocker for the Lean partition theorem — the absence of `HeytingAlgebra (Subobject _)` on elementary topoi in Mathlib4 — is tracked at [`lean/HEYTING-GAP.md`](lean/HEYTING-GAP.md) (sorry-state-and-cite posture; three technical paths documented).
