...
2. **`HeytingAlgebra (Subobject _)` for topoi** — Mathlib upstream gap (triaged 2026-05-17 with Edward van de Meent on Zulip; confirmed absent, no in-flight PR; closest existing scaffold is [`edegeltje/CwFTT`](https://github.com/edegeltje/CwFTT) on the `(X ⟶ Ω)` register, ~60–70 % operator-level complete). Current posture, threshold conditions for upgrading to active collaboration, and the six Heyting-gated `sorry`s in our tree are enumerated at [`HEYTING-GAP.md`](HEYTING-GAP.md). *This remains the cleanest upstream-PR target; pursuing it actively is gated on Tier 2 trajectory work completing.*
