...
3. **Optimal N-TET convergent denominators (Phase 1).** **Kernel-checked:** `qConv_first_six` in `Examples/PythagoreanComma.lean`, with certified log bounds (`MusicKernelLogBounds.lean`) and CF floor lemmas (`MusicKernelCfFloors.lean`). Phases 2–3 — (C1) best-approximation-of-the-second-kind and (C2) strict record-holder equivalence — remain open. See [`../validation/claims/optimal-ntet-continued-fraction.md`](../validation/claims/optimal-ntet-continued-fraction.md).
