...
- The **opposition-figure / bridge track** (July 2026) — the Lean side of the unification spine [`../preprints/opposition-figure/`](../preprints/opposition-figure/): the bridge theorem welding the two stacks (`Positions/OrdinaryKernel.lean` — the partition at `Y` is non-degenerate iff the kernel image is an ordinary element of `Sub(D Y)`, and ordinary kernels force the Z₆ order-embedding), the opposition figure (`Examples/OppositionFigure.lean` — the six landmarks of Aristotle's square under Heyting negation are pairwise distinct iff the element is ordinary; classical/regular/dense collapse laws; the skeleton theorem; the tritone identity on `Div12`; the Blanché-hexagon non-isomorphism), and the Aristotelian relation profile (`Examples/OppositionRelations.lean` — A–E contrariety at ordinary elements, three strict subalternations, the Stone-identity dichotomy for E–I and I–U including intuitionistic unconnectedness, the full profile on `Z₆` at the tritone). Outside the ordinary-elements comparator scope; see `additional_tracks` in [`../formalization.yaml`](../formalization.yaml).
...
## Changelog
- 2026-07-29 (afternoon): Aristotelian relation profile of the opened square — `Examples/OppositionRelations.lean`. At an ordinary element: A–E are contraries never contradictories; three strict subalternations A < I, A < U, E < U; E–I and I–U split on the Stone identity at `a` (contradictory + subcontrary if it holds, contrary + unconnected if it fails); full profile on `Z₆` at the tritone. Axiom audit at most `propext` (Boolean contrast carries all three). Spine §5.1 and audit table updated.
- 2026-07-29: Opposition-figure / bridge track added. `Positions/OrdinaryKernel.lean` welds the topos and lattice stacks: the four morphism-level cell predicates are the lattice cells at the image subobject (`Iff.rfl` for I/D/E; the image-API equivalence for R), the partition at `Y` is non-degenerate iff `kernelImage Δ Y` is ordinary (`partition_nondegenerate_iff_kernel_ordinary`), occupied cells force an ordinary kernel, and ordinary kernels force the `Div12 ≅ Z₆` order-embedding into `Sub(D Y)`. `Examples/OppositionFigure.lean` re-reads `ladderEmbed` as the degraded square of opposition: T1 (`oppositionFigure_injective_iff` — six landmarks pairwise distinct iff ordinary), the classical/regular/dense collapse laws, the skeleton theorem (`oppositionFigure_skeleton`), the tritone identity (`oppositionFigure_tritone_eq_id`), and the Blanché refutation (`BlancheHexagon.not_orderIso_div12`). Axiom audits at most `propext, Classical.choice, Quot.sound`; spine document at `preprints/opposition-figure/paper.md`; track registered in `formalization.yaml` `additional_tracks` (outside the ordinary-elements comparator scope).
