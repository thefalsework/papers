...
**Status (2026-05-20).** `lake build`-green.  The four-position partition theorem (`four_position_partition`) is kernel-checked with no `sorry` on its dependency path; `#print axioms` reports only the three standard Mathlib axioms (`propext`, `Classical.choice`, `Quot.sound`).  The `HeytingAlgebra (Subobject _)` construction it requires has been built locally at [`Heyting/SubobjectInstance.lean`](FalseWorkPapers/Heyting/SubobjectInstance.lean) following Mac Lane–Moerdijk IV.6 Proposition 2, and submitted upstream as Mathlib PR [#39618](https://github.com/leanprover-community/mathlib4/pull/39618) (opened 2026-05-20, CI green, awaiting review).  One `sorry` remains in the entire formalization tree: the substantive `refusal_residue` in [`Refusal.lean`](FalseWorkPapers/Positions/Refusal.lean) — framework-level (not Heyting-related; requires an additional hypothesis on `Δ` that the framework has not yet specified), parked for a separate session.  The earlier "sketch in flight" framing below is preserved as historical record.
