...
* The Nishimura enumeration ("one-generated ⇒ every element is a term
  value") is not formalized *in this file*; it is the [C] input here.
  **However, it is now discharged downstream**: `Examples/NishimuraNormalForm.lean`
  proves the term-identity tables (closure of the ladder set under `⊔`, `⊓`,
  `⇨`, `ᶜ`) and from them `generatedBy_isLadderValue` — every element of the
  subalgebra generated by `g` is a ladder value — and the [C]-free law
  `nishimura_kernel_unique_of_generated`.  So [C] no longer stands over the
  central weld; the conditional form below is kept as the kernel-checkable
  characterization that the discharge then feeds.
