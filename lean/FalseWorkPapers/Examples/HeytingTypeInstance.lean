...
/-! Nishimura normal form — Phase 3 (COMPLETE): the implication table.  All
relations are proved (kernel-checked); the terminal cell families, the two
structural reductions, and the final well-founded assembly are below.  A wrong
implication relation cannot typecheck, so these gate-check the table's
correctness.  `isLadderValue_himp` closes the ladder set under `⇨`; together with
`isLadderValue_sup` and `isLadderValue_inf` (and `isLadderValue_compl`,
`isLadderValue_top`/`bot`) the ladder values are a Heyting subalgebra — the
subalgebra generated by `g` is exactly the Rieger–Nishimura ladder.  This is the
[K]-discharge of the central weld. -/
...
#print axioms FalseWork.Lattice.nishimuraTerm_himp_eq_self_odd
#print axioms FalseWork.Lattice.nishimuraTerm_even_eq_join
#print axioms FalseWork.Lattice.isLadderValue_himp_terms
#print axioms FalseWork.Lattice.isLadderValue_himp

/-! Nishimura enumeration [C] DISCHARGED.  `generatedBy_isLadderValue` is
Nishimura's one-variable normal form — every element of the subalgebra generated
by `g` is a Rieger–Nishimura ladder value — proved from the four closure theorems
rather than cited.  `nishimura_kernel_unique_of_generated` is the kernel-uniqueness
law with the cited enumeration hypothesis replaced by the genuine one-generated
property, so the central weld no longer depends on [C].  Both must report only the
standard axioms. -/
