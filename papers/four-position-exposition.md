...
- **The law** (`nishimura_kernel_unique`, [K] with the hypothesis explicit): any Heyting algebra whose elements are Nishimura term values in an ordinary generator `g` has `g` as its **unique** four-cell kernel. **The enumeration hypothesis is now itself [K]** (`generatedBy_isLadderValue`, 2026-06-17 — *one-generated ⟹ every element is a term value*, proved from the closed Rieger–Nishimura tables, no longer the classical [C]), so `nishimura_kernel_unique_of_generated` covers **every `Z_n` (`n ≥ 6`) and the full infinite Rieger–Nishimura lattice `F(1)` itself** (one-generated by construction) without citation: the free Heyting algebra on one generator has the free generator as its forced kernel. The `n = 6, 7, 8` checks are now samples of a theorem, not the theorem.
...
2. **The four-position structure has a minimum complexity cost, and it is a law.** `Z5.no_kernel` is exhaustive: below six elements the four positions cannot all be occupied, at six they can, uniquely. The partition is not an imposed taxonomy — it switches on at a precise, computable size, with the character of a threshold. The smallest structure that pays the cost is exactly the one Western tonality uses. [K] *Prior-art note (2026-06-10):* via the trichotomy's ordinary-element identification, the `n ≤ 5` half of the threshold is already observed in Citkin 2024 (`Z_2`–`Z_5` contain no ordinary elements; `|A| > 5` for ordinary generators, Prop. 4(c)) — so that half is a kernel-checked re-derivation of a known observation, not a new fact. The uniqueness at and above the threshold was adjudicated by Citkin himself (pers. comm. 2026-06-11): no written proof known to him, folklore-obvious via the quotient picture — and, found the same day, the *statement* appears without proof in his own preprint (arXiv:2512.05633, Dec 2025, p. 13). Its standing is therefore "statement published as an unproved observation; first written proof apparently the Lean one," not "new theorem." *Upgrade (2026-06-11, `LadderCore.lean`):* the threshold is now a **universal law**, not a fact about the `Z_n` family: *any* Heyting algebra — any size, one-generated or not — with a non-degenerate kernel has at least six elements (`ordinary_card_ge_six`, `allFourCells_card_ge_six`; abstract proof, no enumeration, generalizing Citkin's Prop. 4(c) beyond one-generated algebras). And more: for any ordinary element `g` anywhere, the six elements `⊥, g, ¬g, ¬¬g, g ⊔ ¬g, ⊤` realize the exact Hasse structure of `Div12 = Z_6` (`div12OrderEmbedding`, axioms `propext` only) — **the music lattice order-embeds into every non-degenerate instance**, kernel landing on the tritone slot. Scope guard: order-embedding, *not* subalgebra; the subalgebra strengthening is Citkin's Prop. 3.1 [C] (arXiv:2512.05633), whose formalization needs the Nishimura normal-form theorem — registered [O] in `validation/claims/ladder-core-threshold.md`.
...
4. **As standalone mathematics**: "in any Heyting algebra the four-cell partition is non-degenerate at `a` iff `a` is ordinary (non-zero, non-polar, non-regular); consequently every one-generated Heyting algebra with ordinary generator — every `Z_n` with `n ≥ 6`, and the free Heyting algebra on one generator itself — has the free generator as its *unique* ordinary element; and `Z_6` is the subgroup lattice of `ℤ/12`" is a small, clean result connecting the Rieger–Nishimura ladder to music theory — note-sized, abstractly proved (the enumeration input is classical [C]). Novelty standing after the prior-art exchange (`docs/outreach/citkin-email.md`, reply 2026-06-11): the **uniqueness** statement is asserted without proof in Citkin's own arXiv:2512.05633 (Dec 2025), and he knows of no written proof (pers. comm.) — so its standing is "statement his, in print; first written proof apparently ours"; the **`Z_6 ≅ Div(12)`/`p²q` identifications** remain unadjudicated (his reply did not engage that question; no prior art surfaced). The existence threshold is Citkin's (item 2). Not deep; modest; the identifications possibly new, the uniqueness proof now written.
...
| Universal ≥6 threshold (any Heyting algebra, any kernel) | [K] | `ordinary_card_ge_six`, `allFourCells_card_ge_six` |
| `Div12 = Z_6` order-embeds into every non-degenerate instance | [K] | `div12OrderEmbedding` (`propext` only) |
...
- At six elements and above — every finite truncation, and the infinite algebra itself (a law, proved abstractly; the Nishimura enumeration it once leaned on is now itself kernel-checked, `generatedBy_isLadderValue`, so the law is unconditional): there is **exactly one** place to draw the fence so that all four positions are occupied — at `p` itself, the free generator. And the four occupants are always the same four formulas: `p`, `¬p`, `¬¬p`, `p ∨ ¬p`.
...
- Not that the music topos is *the* canonical music topos, or that the Commitment gate has content yet — open, listed. (The stability of the kernel at every truncation size, previously open, is now the all-n law — and since 2026-06-17 fully unconditional: the Nishimura enumeration it once cited is itself kernel-checked, `generatedBy_isLadderValue`.)
