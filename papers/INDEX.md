...
**v11.9 coherence debt (flagged 2026-06-19).** Three framing fixes the body rewrite should carry, none requiring new content: (a) **five → four.** The "five and only five positions" derivation (§3.4, abstract, §1) now contradicts the kernel-checked **four**-position partition + Commitment gate; reconcile the headline with the [K] theorem. (b) **The two senses of "kernel."** Distinguish the *working* kernel (the four prose criteria — prior/monogenic/inescapable/self-limiting) from the *formal* kernel (an ordinary element of a Heyting algebra). They **provably coincide only in music** (monogenic ↔ one-generated `tritone_generates`; self-limiting ↔ `¬¬k ≠ k`; inescapable ↔ normal form `generatedBy_isLadderValue`) and **can decouple** (`H8`, `unique_ordinary_converse_false`: self-limiting without monogenic) — so a domain may have a working kernel with no formal kernel behind it. The universal claim rides the working kernel; only music rides both. (c) **Register.** Lower title/abstract ("a structural derivation of universal positions") to the body's own validation gradient (§3.2) — one proven instance, candidates graded. The seam is described in [`position-taking-from-the-kernel-up.md`](position-taking-from-the-kernel-up.md) §3; the full treatment belongs in this paper.

**Resolution of (a) — five → four (decided 2026-06-24).** The project decision is recorded in [`four-cell-canonical.md`](four-cell-canonical.md): the **four-cell partition is canonical; Commitment is a within-cell fidelity gate, not a fifth position.** The decision is forced by the formal floor (disjoint/exhaustive at four; the two-parameter unification closed negative) and matches the corpus's own best close readings (Art of Fugue as "Infrastructure at its absolute boundary"). The one substantive reclassification is **Russian Ark** (Commitment-peer-of-Refusal → (Exploitation, commitment-yes), the closure-residue worked to its limit — already the state of the DB via migrations 0170–0173). Propagation is tracked in that note's checklist (assistant prompt, `introduction` curriculum, cinema curriculum + Russian Ark engagement, renderer, this paper's §3.4 rewrite, and the `/guides` four-row position layer). Items (b) and (c) remain open for the body rewrite.
...
**Submission event (2026-08-18).** The arXiv v2 submission (submit/7711078) was **declined by arXiv moderation** ("in need of significant review and revision before it would be considered publishable by a conventional journal" — boilerplate, no substantive feedback), and the account was placed under a journal-reference gate: future submissions require a journal reference/DOI until a publication record exists, at which point arXiv states it will reconsider. Probable triggers, assessed honestly: independent affiliation, AI-assistance disclosure landing in the 2026 LLM-submission flood, and interdisciplinary content in the most defensively moderated categories — the metadata was pattern-matched; the framework was not evaluated. Routing decision: (1) Paper 2's preprint home moves to **PhilSci-Archive** (genre-appropriate) with a **Zenodo** deposit for a DOI'd timestamp of the current version; (2) peer-review targets, in order: *Philosophy & Technology*, *Minds and Machines*, *Synthese*; (3) the planned *Algebra Universalis* extraction of the aperture mathematics (`preprints/aperture/`) now doubles as the account-rehabilitation path, since a journal DOI is what lifts the gate; (4) a future v8.2x revision should absorb this event into the paper's own correction-mode narrative, exactly parallel to the v8.5 treatment of Mathlib PR #39618's closure under the AI-use policy — an institutional boundary-setting recorded as data about the epistemic-dependency landscape, not as grievance. There is a structural irony worth preserving in that revision: the paper *about* epistemic dependency and correction-mechanism maturity was declined by an overloaded human moderation layer pattern-matching against AI involvement.
...
- **[The Unique Ordinary Element of a One-Generated Heyting Algebra, the Subgroup Lattice of ℤ/12ℤ, and a Characterization of n = p²q](../preprints/ordinary-elements-z6/paper.md)** — *Chris Brink, July 2026, preprint not yet submitted.* The pure lattice theory of the mathematics anchor, extracted with no framework vocabulary: the four-region non-degeneracy theorem (non-degenerate iff the element is ordinary), the ≥ 6 threshold and Z₆ order-embedding, the uniqueness of the ordinary element in one-generated Heyting algebras (statement Citkin's, unproved in arXiv:2512.05633; the kernel-checked proof apparently the first written, via a full formalization of Nishimura's one-variable normal form), the `H₈` converse counterexample, `Div12 ≅ Z₆` with the tritone as free generator, the `n = p²q` characterization now proved for every number of prime factors via the exact count formula ∏(aᵢ+1) − ∏aᵢ − 2^k + 1 (least instance 12), and the substitution monoid End(Z₆) ≅ Z₆ whose identity is pinned by the unique ordinary element (every endomorphism satisfies f³ = f, a kernel-checked Ruitenburg instance). Musical reading confined to one flagged section. Kernel-checked in Lean 4 / Mathlib4; prior-art standing adjudicated via the Citkin correspondence ([`../docs/outreach/citkin-email.md`](../docs/outreach/citkin-email.md)).
...
- **[The Opened Square: Aristotle, Spencer-Brown, and the Kernel-Checked Foundation of the Four-Position Lens](../preprints/opposition-figure/paper.md)** — *Chris Brink, July 2026, unification spine (not an arXiv target in current form).* The front door to the formal foundation: welds the topos stack to the lattice stack via the **bridge theorem** (the partition at `Y` is non-degenerate iff the kernel image is an ordinary element of `Sub(D Y)`; `Positions/OrdinaryKernel.lean`), so every theorem of the ordinary-elements preprint becomes a formal statement about the lens's kernels; formalizes the **opposition figure** (`Examples/OppositionFigure.lean`): the six landmarks Aristotle's square generates under Heyting negation are pairwise distinct iff the element is ordinary (T1), with the classical/regular/dense collapse laws explaining the square's 2,300-year silence, the skeleton theorem (each middle landmark inhabits its partition cell), the tritone identity on Z₆, and the machine-refuted Blanché-hexagon identification (T2). Interpretive glosses carried at grade **[A]**, never promoted. Kernel-checked; audit table in §10; two human-review items open (Blanché cover table; the 2012 Birkhäuser chapters).

---

## Next revisions anticipated

- **Paper 2 v8.2x.** Absorb the 2026-08-18 arXiv moderation decline and journal-reference gate (see the submission-event note in Paper 2's entry above) into the paper's correction-mode narrative, parallel to the v8.5 Mathlib-PR treatment; retarget distribution per the routing decision (PhilSci-Archive + Zenodo preprint; *Philosophy & Technology* / *Minds and Machines* / *Synthese* for review).
