...
*Version 0.1 (June 2026); pointer note July 2026. Expository synthesis. This note runs the four-position structure end to end — from the kernel-checked partition theorem at the bottom to the interpretive placement of works (Bach, Berg, Schoenberg, Beckett, and machine-generated process music) at the top — and its single discipline is to mark, at every rung, where the verified floor stops and interpretation begins. Status: the formal floor (§§2–3) is **[K]**, kernel-checked in Lean 4 / Mathlib4, cited with artifacts. Everything from §5 onward — every placement of an actual work or composer in a cell — is **[A]**, a structural reading that does not inherit the floor's certainty. The contribution of the note is the **map between the two**, and the honesty of the seam. The kernel decides what the cells are; it does not decide where Bach, Schoenberg, or Beckett sit in them. **Navigation (July 2026):** the programme front door is now [`field-guide.md`](field-guide.md); the deep music specimen is [`bach-at-the-kernel.md`](bach-at-the-kernel.md) (v0.2, with opened-square landmarks). This note remains the seam-marked placement walk from bedrock to Beckett — not superseded, just no longer the first document to hand a new reader.*
...
**Two senses of "kernel," and where they meet.** This note uses the word in two registers, and the honesty of everything below depends on not conflating them. The **formal kernel** is the ordinary element of §2 — the object the partition theorem is *about*. The **working kernel** is what a practitioner takes a position toward: the inherited generative constraint a domain cannot escape (the four prose criteria — *prior, monogenic, inescapable, self-limiting* — of Paper 1). These are not two unrelated things sharing a word: in music they *provably coincide*. "Monogenic" *is* one-generated (`tritone_generates`); "self-limiting" *is* `¬¬k ≠ k`, the comma (the ordinary condition itself); "inescapable" *is* the normal form — every element is a term in the generator (`generatedBy_isLadderValue`). So at the **Bach end** of this note the working kernel and the formal kernel are the same object, which is exactly why the music placements may ride the §2 floor. But the coincidence is **earned, not free**: `H8` (`unique_ordinary_converse_false`) is a Heyting algebra that is *self-limiting* (a unique ordinary element) yet *not monogenic* (not one-generated by it) — proof that the two faces of "kernel" can come apart. So a domain may have a working kernel whose formal kernel has not been established, or does not exist. At the **Beckett end** of this note that is exactly the situation: literature has a working kernel (inherited form, to be refused) and *no* formal kernel at all. The full treatment of this seam — the four criteria, the `H8` decoupling, the per-domain gradient — belongs in Paper 1 and `comma-formal-structure-note.md`; what matters here is that the word is doing two jobs, they meet only in music, and the placements inherit their floor accordingly.
...
- **Literature has only the working kernel, not the formal one.** The music placements sit above a real formal object (`Div12`, the subobject lattice), where — per §3 — the working kernel and the formal kernel provably coincide. The Beckett placement does not: literature has a *working* kernel (inherited form, there to be refused) but **no formal kernel** — no kernel-bearing lattice of literary works. Applying *Refusal* to Beckett is transporting a vocabulary the theorem grounds *in music* to a domain where only the prose face of the kernel is present — metaphor calibrated by the music case, nothing more. The further from `Div12`, the thinner the floor; over literature there is no formal floor at all, only the calibrated word. (And by the `H8` decoupling of §3, this is not a mere "not yet formalized" — a working kernel need not have a formal kernel behind it.)
