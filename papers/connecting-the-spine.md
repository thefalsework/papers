...
- **The threshold.** Below cardinality 6 the partition is degenerate — no element of `Z_5` (exhaustively, `Z5.no_kernel`) makes all four cells inhabited. At 6 and above, exactly one element does, and it is the **free generator**, with the same four witnesses at every level: `g, ¬g, ¬¬g, g ⊔ ¬g`. **[K]** at `n = 6, 7, 8`.
