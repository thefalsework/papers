...
### 5.7 The Nishimura normal form — **active target (2026-06-17)**
With the ratio end now closed (row 9a — (C1)/(C2) kernel-checked) and the weld core solid (rows 20b–20g), the highest-leverage open item in the corpus is the **Nishimura normal form**: that every element of the subalgebra generated by an ordinary `g` is a Nishimura term value in `g` — the Rieger–Nishimura meet/join/implication tables as derived identities. Closing it would (i) **discharge the [C]** on the all-n kernel law (row 20d), making the math anchor unconditional, and (ii) upgrade `div12OrderEmbedding` (row 20g) from order-embedding to **subalgebra** — Citkin's Prop. 3.1 in full, in every non-degenerate instance. Phased plan and Phase-1 progress at `validation/claims/nishimura-normal-form.md`; Lean at `Examples/NishimuraNormalForm.lean`. This is where the next push is aimed: of the remaining open items it is the one whose closure would most change the framework's standing, because it converts the program's central weld from "conditional on a cited classical enumeration" to fully kernel-checked.
...
The shape of the whole: **one kernel-checked horizontal theorem, now with a kernel-checked music-topos instance, a kernel-checked *qualitative* Diophantine floor underneath it, and a kernel-checked optimal-temperament characterization at the ratio end** (row 9a), joined to a vertical program that remains, deliberately and explicitly, structural analogy and open specialist work. The rank-1↔rank-2 *unification* of that floor is open (§2.3, §5.3), not part of the kernel-checked core. The chain is now solid-core and solid-at-the-ratio-end; the single open item whose closure would most change the standing of the core is the Nishimura normal form (§5.7), which is the active target.
