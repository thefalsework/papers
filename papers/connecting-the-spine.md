...
**Register:** Synthesis note. Ties the formal results of Papers 1, 3, 4, 5 and the four-position-partition preprint into a single chain, and reports the new kernel-checked results that connect the music anchor to the central theorem without external validation. Markdown authoritative. **July 2026:** the historical / lens weld (Aristotle opened square, bridge theorem, relation profile) lives in [`../preprints/opposition-figure/`](../preprints/opposition-figure/) and is summarized in §3.5 below — this note remains the music↔Diophantine↔topos technical weld; that preprint is the Aristotle→lens theorem spine. The humanities front door is [`field-guide.md`](field-guide.md).
...
- **The all-n law** (`Examples/NishimuraKernelLaw.lean`). The sample became a theorem: (i) in *any* Heyting algebra, all four cells are inhabited at `a` iff `a ≠ ⊥ ∧ ¬a ≠ ⊥ ∧ ¬¬a ≠ a` (`allFourCellsInhabited_iff`, unconditional **[K]** — the abstract reason Boolean algebras and `Z_5` have no kernel); and (ii) any Heyting algebra whose elements are Nishimura term values in an ordinary generator has that generator as its **unique** four-cell kernel (`nishimura_kernel_unique`, **[K]** with the hypothesis explicit). **The enumeration hypothesis is now itself [K]** (`generatedBy_isLadderValue`, 2026-06-17 — *one-generated ⟹ every element is a term value*, proved from the closed RN meet/join/implication tables, §5.7), so `nishimura_kernel_unique_of_generated` covers every `Z_n` (`n ≥ 6`) and the full infinite Rieger–Nishimura lattice **without the [C] citation**. Consistency weld: `Div12`'s hypothesis is discharged inside Lean three ways (`Div12.kernel_unique` by `decide`, `kernel_unique_via_law` by enumeration, `kernel_unique_via_subalgebra` by the [C]-free generation route), all agreeing.
...
- **The ladder core: the threshold is universal and the music lattice sits inside every instance** (`Examples/LadderCore.lean`, 2026-06-11; pre-registered in `validation/claims/ladder-core-threshold.md`; triggered by Citkin's arXiv:2512.05633, whose Prop. 3.1 — *any Heyting algebra with an ordinary element contains a `Z_{n≥6}` subalgebra* — is the [C] statement this kernel-checks a fragment of). Three upgrades. (i) **The six-element threshold became a universal law**: any Heyting algebra — finite, any size, one-generated or not — with an ordinary element (equivalently, a non-degenerate four-cell kernel) has at least six elements (`ordinary_card_ge_six`, `allFourCells_card_ge_six`). Abstract proof, no enumeration; *stronger on both sides than Citkin 2024 Prop. 4(c)* (stated there for generators of one-generated algebras). `Z5.no_kernel` is now a corollary of a law, re-derived independently (`Z5.no_kernel_via_threshold`). (ii) **The music lattice order-embeds into every non-degenerate instance** (`div12OrderEmbedding`, axioms `propext` only): for any ordinary `g` anywhere, the six elements `⊥, g, ¬g, ¬¬g, g ⊔ ¬g, ⊤` realize the exact Hasse structure of `Div12 = Z_6`, incomparabilities included, kernel ↦ tritone slot. *Scope guard*: order-embedding, **not** subalgebra — the subalgebra strengthening is exactly Prop. 3.1 [C], whose formalization needs the Nishimura normal-form theorem (registered [O]). (iii) **H8's anatomy resolved**: the subalgebra generated by `H8`'s unique ordinary element is exactly `Z_7` (`h8_ladder_core` — explicit Heyting embedding, range = the seven elements `≠ d`), so the converse-counterexample is `Z_7` plus one dense element: Prop. 3.1 instantiated at the very algebra that refuted the converse. Two side results: the tritone is the **unique** generator of the music lattice (`Div12.generator_unique`, instance of Citkin's pers.-comm. remark that one-generated algebras of cardinality > 5 have unique generators); and the **dense-bottom lemma** (`no_ordinary_of_least_nonzero`): a least nonzero element kills all ordinariness — whence Citkin's prohibited algebra `P2 ≅ 2 + Z_7` carries *no* kernel while `P1 ≅ Z_7` carries exactly one, so prohibitedness (hereditary structural completeness) and kernel-bearing do not align.
...
### 3.5 The lens weld: bridge theorem + opened square (2026-07) — [K], pointer

The June weld of this note joins **music ↔ free Heyting algebra ↔ Diophantine floor**. A July development welds the **interpretive lens** into the same ordinary-element language — without changing the music results above. Full write-up: [`../preprints/opposition-figure/paper.md`](../preprints/opposition-figure/paper.md). Humanities walk: [`field-guide.md`](field-guide.md), [`bach-at-the-kernel.md`](bach-at-the-kernel.md).

- **Bridge** (`Positions/OrdinaryKernel.lean`). The four morphism-level cell predicates *are* the lattice cells at the image subobject relative to `kernelImage Δ Y`. The partition at `Y` is non-degenerate iff that kernel image is an ordinary element of `Sub(D Y)` (`partition_nondegenerate_iff_kernel_ordinary`). Occupied cells force an ordinary kernel; an ordinary kernel forces the `Div12 ≅ Z₆` order-embedding into `Sub(D Y)`. So every theorem of the ordinary-elements / all-n track is, formally, a statement about the lens's kernels. **[K]**
- **Opened square** (`Examples/OppositionFigure.lean`). Aristotle's square at `a` generates six landmarks `⊥, ¬a, a, ¬¬a, a∨¬a, ⊤`. They are pairwise distinct iff `a` is ordinary (`oppositionFigure_injective_iff`) — the same condition as partition non-degeneracy. The four middle landmarks inhabit the four cells one-for-one (`oppositionFigure_skeleton`): that is the dictionary of §1 / §3.4, re-read as the degraded square. At the tritone the figure is the identity on `Div12`. Blanché's classical hexagon is not order-isomorphic to `Z₆` (`BlancheHexagon.not_orderIso_div12`). **[K]**
- **Relation profile** (`Examples/OppositionRelations.lean`). At an ordinary `a`, A–E are contraries (never contradictories); three strict subalternations; E–I and I–U split on the Stone identity at `a` (including Demey–Smessaert unconnectedness when it fails). Full profile on `Z₆` at the tritone. **[K]**

This note does not restate those proofs. It records that the horizontal theorem now has a **historical and lens-facing face** as well as a music-facing face — both keyed to ordinariness.
...
### 5.7 The Nishimura normal form — **CLOSED (2026-06-17)**
This was the highest-leverage open item in the corpus; it is now **kernel-checked and closed**, and the central weld is **unconditional**. The normal form — every element of the subalgebra generated by an ordinary `g` is a Nishimura term value in `g` — is proved by closing the Rieger–Nishimura **meet/join/implication tables** as derived identities (`isLadderValue_sup` / `_inf` / `_himp`, plus the complement table; `Examples/NishimuraNormalForm.lean`, all `sorry`-free, axioms `propext, Quot.sound` — no `Classical.choice`). The two leverage points are both realized:

(i) **The [C] on the all-n kernel law (row 20d) is discharged.** `generatedBy_isLadderValue` proves *one-generated ⟹ every element is a term value* — exactly the former Nishimura-enumeration citation — by structural induction on the generated subalgebra using the four closure theorems. `nishimura_kernel_unique_of_generated` is the kernel law with the cited hypothesis replaced by the genuine one-generation property; the math anchor no longer depends on [C].

(ii) **The subalgebra form is realized for the music object, with a correctness flag on the abstract version.** Scoping (see `validation/claims/nishimura-normal-form.md`, "Phase 4 scope") showed the *fully-abstract* upgrade — "`div12OrderEmbedding` is a subalgebra embedding for every ordinary `g`" — is **false**: the six-element image is not `⇨`-closed in `F(1)` (`gᶜᶜ ⇨ g = x₅` escapes), so it needs a collapse hypothesis. The honest, hypothesis-free statement is concrete and done: `Examples.Div12.tritone_generates` — the tritone generates the *whole* music lattice as a Heyting subalgebra — with `kernel_unique_via_subalgebra` giving a third, [C]-free derivation of the tritone as unique kernel. The decisive technical step was that **odd targets of the implication table are terminal** (`nishimuraTerm_himp_eq_self_odd`), which collapses the two-index induction to a one-dimensional even-target climb.

The remaining open sub-item is optional packaging only (a bundled `HeytingHom`/`Subalgebra` object under an explicit collapse hypothesis); it carries no new mathematics. The weld is converted from "conditional on a cited classical enumeration" to fully kernel-checked — the change in standing this section was written to anticipate has happened.
...
| 9a | **Optimal N-TET ↔ continued-fraction convergents of `α = log₂(3/2)`.** (C1) best approximation of the second kind for convergents (proved for a general irrational `ξ`); (C2) the Pythagorean-comma-optimal temperaments are exactly the convergent denominators (`1, 2, 5, 12, 41, 53, …`). Math classical (Khinchin best-approx of the second kind, Hardy & Wright §10.15); the contribution is the formalization — (C1) appears absent from Mathlib (only Legendre's converse) — and the application to the comma. The ratio end of the chain | **[K]** | `convergent_best_approx_second_kind`, `best_tet_iff_record_convergent_denominator` |
...
| 20d | **The all-n kernel law.** (i) Kernel trichotomy, general and unconditional: in *any* Heyting algebra all four cells are inhabited at `a` iff `a ≠ ⊥ ∧ ¬a ≠ ⊥ ∧ ¬¬a ≠ a` — i.e. iff `a` is **ordinary** in the standard sense (Citkin §2.1; the `Z_2`–`Z_5` no-ordinary-element observation is already in that paper, so the threshold's lower half is known; the uniqueness half: statement asserted without proof in Citkin's own arXiv:2512.05633 (Dec 2025), adjudicated no-known-proof by him, pers. comm. 2026-06-11 — first written proof is apparently the Lean one) (explains why Boolean algebras and `Z_5` have no kernel). (ii) The law, hypothesis now discharged: any Heyting algebra whose elements are Nishimura term values in an ordinary generator `g` has `g` as its **unique** four-cell kernel. **The Nishimura enumeration is now [K], not [C]** (`generatedBy_isLadderValue`, 2026-06-17): *one-generated ⟹ every element is a term value*, proved from the closed RN tables — so `nishimura_kernel_unique_of_generated` is the kernel law with the enumeration discharged, covering **every `Z_n` (`n ≥ 6`) and the full infinite Rieger–Nishimura lattice `F(1)`** (one-generated by construction) without citation. Consistency welds: `Div12`'s hypothesis discharged inside Lean three independent ways — exhaustive `decide` (`Div12.kernel_unique`), the enumeration route (`Div12.kernel_unique_via_law`), and the [C]-free subalgebra route (`Div12.kernel_unique_via_subalgebra`) — all agreeing | **[K]** (law *and* enumeration) | `allFourCellsInhabited_iff`, `nishimura_kernel_unique_of_generated`, `generatedBy_isLadderValue`, `four_le_nishimuraTerm`, `Div12.kernel_unique_via_law` |
...
| 20g | **The ladder core (Prop. 3.1 fragment).** (i) Universal threshold: *any* Heyting algebra with an ordinary element / non-degenerate kernel has ≥ 6 elements — abstract, unconditional, generalizing Citkin 2024 Prop. 4(c) beyond one-generated algebras; `Z5.no_kernel` re-derived as corollary. (ii) `Div12 = Z_6` **order-embeds** into every Heyting algebra with an ordinary element (kernel ↦ tritone slot; `propext` only). The *subalgebra* strengthening was scoped (2026-06-17): the fully-abstract "order-embedding is a subalgebra embedding for every ordinary `g`" is **false** (the six-element image is not `⇨`-closed in `F(1)` — `gᶜᶜ ⇨ g = x₅` escapes), so it needs a collapse hypothesis; but the honest concrete form is **[K]** — `Div12.tritone_generates`: the tritone generates the whole music lattice as a Heyting subalgebra, Citkin Prop. 3.1 for the music object, no hidden hypothesis. (iii) `H8`'s ladder core is exactly `Z_7` (Heyting embedding, range = the 7 elements `≠ d`): the converse-counterexample is `Z_7` + one dense element. (iv) Tritone = unique generator of `Div12`. (v) Dense-bottom lemma: least nonzero element ⟹ no ordinary elements; so prohibited `P2 ≅ 2+Z_7` has no kernel while `P1 ≅ Z_7` has one — prohibitedness ⊥̸ kernel-bearing. Pre-registered `ladder-core-threshold.md`; V(Z_6)-primitiveness route registered [O], blocked on P3–P5 diagrams | **[K]** (i–v; **normal form now closed**, see §5.7 / row 20d); [C] (P1/P2 identifications); [O] (primitiveness only) | `ordinary_card_ge_six`, `allFourCells_card_ge_six`, `div12OrderEmbedding`, `tritone_generates`, `h8_ladder_core`, `Div12.generator_unique`, `no_ordinary_of_least_nonzero` |
...
| 22 | **Bridge + opened square (lens weld, 2026-07).** Partition non-degeneracy at `Y` iff `kernelImage` ordinary; cell predicates = lattice cells at image; ordinary kernel ⟹ `Div12` order-embeds. Six landmarks of Aristotle's square under Heyting negation pairwise distinct iff ordinary; skeleton = four middle landmarks in the four cells; tritone identity; Blanché hexagon ≇ `Z₆`. Relation profile (A–E contrariety; Stone dichotomy). Full narrative: `preprints/opposition-figure/` | **[K]** | `partition_nondegenerate_iff_kernel_ordinary`, `oppositionFigure_injective_iff`, `oppositionFigure_skeleton`, `oppositionRelationProfile`, `BlancheHexagon.not_orderIso_div12` |

The shape of the whole: **one kernel-checked horizontal theorem, now with a kernel-checked music-topos instance, a kernel-checked *qualitative* Diophantine floor underneath it, a kernel-checked optimal-temperament characterization at the ratio end** (row 9a), **and (July 2026) a kernel-checked lens/historical face keyed to the same ordinariness condition** (row 22), joined to a vertical program that remains, deliberately and explicitly, structural analogy and open specialist work. The rank-1↔rank-2 *unification* of that floor is open (§2.3, §5.3), not part of the kernel-checked core. The chain is now solid-core and solid-at-the-ratio-end; the item whose closure would most change the standing of the core — the Nishimura normal form (§5.7) — is now **closed**, discharging the [C] on the all-n law and making the central weld unconditional. The keystone is in.
...
Positions/OrdinaryKernel.lean             partition_nondegenerate_iff_kernel_ordinary
                                          (lens bridge — see §3.5)
Examples/OppositionFigure.lean            oppositionFigure_injective_iff,
                                          oppositionFigure_skeleton
Examples/OppositionRelations.lean         oppositionRelationProfile
```
