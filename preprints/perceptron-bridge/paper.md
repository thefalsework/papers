...
**Instrument, with its honesty note registered up front.** On the frame Opens X, double negation ¬¬ (interior of closure) is a genuine nucleus with no resolution parameter, and the quantity of interest is the regularization residual [U, ¬¬U]. At grid resolution ε, morphological closing (dilate then erode, 3×3) **estimates** this residual: the closing residual C_ε(U) ∖ U is the phantom estimate at that resolution. *Erratum (2026-09-15): an earlier version of this paragraph asserted that ε-closing is itself a nucleus. It is not — closing is extensive, monotone and idempotent but fails binary meet-preservation (disjoint sets can have intersecting closings), the failure mode this repository kernel-checks as `tritoneClosure_not_nucleus`. The nucleus is ¬¬; ε-closing is its estimator. No measured number changes.* The resolution sweep samples a one-parameter estimator family chosen for computability. A negative result therefore means *this family sees nothing*, not that no observer does.
