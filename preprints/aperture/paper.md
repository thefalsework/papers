...
**What remains open [O].** The extension of Theorem 5.1 and the latency characterization beyond finite products of chains — general finite distributive lattices, then general Heyting algebras — is untouched. The derivation's three steps are all kernel-checked, and the assembled formula is [K] on two-prime lattices (§5, §8); the r > 2 iteration of the assembly is the remaining formalization gap, alongside the two completeness instances already done (Results 4.1, 6.1).
