...
**Version.** Draft v0.4, September 2026 — not yet posted. v0.3 (2026-08-24) corrects
...
v0.4 (2026-09-09) adds §7, the **co-aperture** — the dual ledger, measuring what
each observer destroys rather than which observers preserve — with a registered
spec committed before the Lean work (`coaperture-spec.md`, same directory),
closed forms at every arity, exact multiplicativity on products, and
...
The motivating reading — nuclei as observers, Fix(j) as the world at an observer's resolution, the aperture as *which observers see a distinction's position space open* — is stated in §8, as an analogy and after the mathematics. The theorems and computations stand without it.
...
**What remains open [O].** The extension of Theorem 5.1 and the latency characterization beyond finite products of chains — general finite distributive lattices, then general Heyting algebras — is untouched. The formalization gap the v0.2 text named here is closed: the r > 2 iteration was completed the day of the v0.3 correction, and the assembled formula is now [K] on all divisor lattices (§5, §9; `aperture_closed_form_pi`), with both correction witnesses kernel-checked. What the correction episode leaves open at computed grade is the corrected characterization itself (Result 6.3) as a general symbolic equivalence — its formula is [K], its witnesses are [K], but the "iff" over all exponent vectors is verified, not derived in the kernel. The episode remains a live demonstration that on this terrain prose summaries fail exactly where kernel-checking refuses to.

## 7. The co-aperture: the dual ledger (added at v0.4)

The aperture counts the observers under which a kernel's four-fold survives. It says nothing about what any observer *destroys*. This section defines the dual invariant and reports its closed forms and its independence from the aperture — all kernel-checked against a spec registered before the formalization (`coaperture-spec.md`, this directory, 2026-09-09; the spec's hand-computed expectations E0–E5 were verified with no deviations of content).

**Definition 7.1 (phantom mass) [K].** For a nucleus j and kernel k, the **phantom mass** of k under j is |[k, j(k)]| — the cardinality of the interval from k to its image (`phantomMass`, `lean/FalseWorkPapers/Lattice/CoApertureClosedForm.lean`). Always ≥ 1; equals 1 exactly when j fixes k.

The name is licensed by a two-line lemma:

**Lemma 7.2 (confusion class) [K] (`IsNucleus.le_apply_iff`).** For k ≤ x: x ≤ j(k) iff j(x) = j(k). The interval [k, j(k)] is exactly the set of elements at or above k that the observer cannot distinguish from k. Phantom mass counts a confusion class — *confident false presence*, not absence: everything in the interval is reported by the observer as k. (Proved on any meet-semilattice from inflation, monotonicity, and idempotence alone.)

**Definition 7.3 (co-aperture) [K].** coAp(k) = Σⱼ phantomMass(j, k), summed over all nuclei on H (`coaperture`). Where the aperture asks *which observers keep the four-fold open at k*, the co-aperture asks *how much confusion the observer population manufactures at k*.

**Theorem 7.4 (closed forms) [K].**

1. **Chains** (`coaperture_chain_add`): on the chain Fin(m+1) with kernel exponent e, coAp(e) + 2^e = 2^{m+1}. Engine: for e ≤ x, the observers conflating x with e are exactly the top-sets avoiding [e, x), and they number 2^{m−(x−e)}; the sum is geometric.
2. **Exact multiplicativity** (`coaperture_pi`): on any finite product of finite semilattices-with-top, coAp(k) = ∏ᵢ coAp(kᵢ). The contrast with Theorem 5.1 is structural: the aperture needs inclusion-exclusion to cross coordinates (ordinariness is a *conjunction* of a non-density and a non-regularity witness that may live on different chains), while phantom mass is a plain interval count and intervals in products are products of intervals. **The destruction ledger composes more simply than the survival ledger.**
3. **Divisor lattices** (`coaperture_closed_form_pi`): on Div(∏ pᵢ^{aᵢ}) with kernel exponents (kᵢ), coAp(k) = ∏ᵢ (2^{aᵢ+1} − 2^{kᵢ}), over ℤ, at every arity. Instances: Div12, kernel 2 = (1,0): (8−2)(4−1) = 18. Bottom maximizes co-aperture (everything conflates upward from ⊥); ⊤ minimizes it (|nuclei| exactly, mass 1 each).

**Theorem 7.5 (independence, both directions) [K] (four `decide`d witnesses).** On Div24, kernels 2 and 4 have **equal apertures** (3 and 3, per §4) and **unequal co-apertures** (42 and 36). On Div72, kernels 4 and 6 have **equal co-apertures** (84 and 84) and **unequal apertures** (9 and 6). Neither invariant determines the other. What survives observation and what observation destroys are independent coordinates on kernels — a kernel is not positioned by its survival profile alone.

**Remark 7.6 (the geometric face of phantom mass, and its registered negative result).** In Opens X, double negation is interior-of-closure (regularization), and the ¬¬-remainder of a composed threshold region is a phantom in exactly the sense of Lemma 7.2 — a locus the healed region treats as interior which no classical reduction can see (kernel-checked for a worked two-layer circuit: the remainder is exactly the gluing wall; `Examples/PerceptronRegular.lean`, `composed_seam`). Whether *trained* neural networks carry substantial phantom mass at data-relevant scales was tested under a pre-registered protocol and answered **no** — the registered kill condition triggered, with a codimension-2 mechanism (the phantom of trained ReLU nets concentrates where polytope faces meet) and a confirmed depth-scaling survivor at fixed neuron budget. The full account, with the bridge theorems and the pilot, is the companion note `preprints/perceptron-bridge/`; the co-aperture's own mathematics above is untouched by the negative result.

## 8. The observer bridge, stated as an analogy
...
| confusion-class lemma (Lemma 7.2) | [K] (`IsNucleus.le_apply_iff`, `CoApertureClosedForm.lean`; any meet-semilattice) |
| co-aperture chain closed form (Thm 7.4.1) | [K] (`coaperture_chain_add`, ibid.) |
| co-aperture exact multiplicativity (Thm 7.4.2) | [K] (`coaperture_pi`, ibid.; any finite product) |
| co-aperture divisor closed form (Thm 7.4.3) | [K] (`coaperture_closed_form_pi`, ibid.; every arity) |
| aperture/co-aperture independence, both directions (Thm 7.5) | [K] (four `decide`d witnesses on Div24 and Div72, ibid.) |
| co-aperture beyond products of chains; extremal/comparative structure | [O] |
