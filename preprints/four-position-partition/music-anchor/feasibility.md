...
| Layer | Claim                                              | Status                          |
|-------|----------------------------------------------------|---------------------------------|
| L     | Divisor lattice of 12 is non-Boolean Heyting; four-position partition non-vacuous at tritone kernel; tritone is the *unique* fully non-vacuous kernel choice | **Kernel-checked in Lean** (`FalseWork.Lattice.lattice_four_position_partition` abstract, `FalseWork.Lattice.Examples.Div12.music_anchor_witness` concrete; audit lines in `Examples/HeytingTypeInstance.lean`). The "equivalently the subgroup lattice of `Z/12`" reading is now itself kernel-checked: `pcset_realizes_subgroup_lattice` (`Examples/DivisorLattice12Distinction.lean`) exhibits the six elements as the six transposition-symmetric pitch-class subgroups of `Z/12` with order = inclusion and meet = intersection. **Uniqueness verified computationally** in §5 of `wolfram/music-anchor/layer-t-d-checks.wl`. |
...
| D     | A concrete distinction structure on T with the right kernel image lifts Layer L to Theorem 5.1 | **Lattice-level slice now kernel-checked; topos lift still deferred.** 23 closure operators total on `L`; 5 substantive ones (4 tritone-closing + 1 whole-tone-closing). The minimal tritone-closing candidate `{2,12}` is now realized in Lean as `FalseWork.Lattice.Examples.Div12.tritoneClosure` and proved to be a closure operator (extensive/monotone/idempotent) whose kernel image `tritoneClosure ⊥` is the tritone, a non-regular element — `tritoneClosure_is_distinction_slice` (`Examples/DivisorLattice12Distinction.lean`). This de-arbitrarizes the kernel `a = ⟨6⟩` used by `music_anchor_witness`: it is now the `Im(η)` of a concrete idempotent operation, not a stipulation. The kernel-checked lift constructor `FalseWork.Positions.DistinctionStructure.ofIdempotentMonad` is in place. What remains deferred is only the **topos-level** lift: realizing `tritoneClosure` as an idempotent monad on a concrete elementary topos `T` with `Sub_T(1) ≅ L` and recovering it as `kernelImage`. That step is the subject of `mazzola-bridge-note.md` §5. |
...
- **Topos-level Layer-D witness on `Sh(L)` or `Set^{P^op}`.** The Lean-level lift constructor `FalseWork.Positions.DistinctionStructure.ofIdempotentMonad` is in place; the four lattice-level slices are enumerated; and the minimal one `{2, 12}` is now kernel-checked as a closure operator (`tritoneClosure`, `Examples/DivisorLattice12Distinction.lean`) with kernel image at the tritone. What remains is only to lift it to a full *topos-level* idempotent monad on a concrete `T` with `Sub_T(1) ≅ L` and verify the lift recovers it as `kernelImage`. This is the open item framed for specialist correspondence in `mazzola-bridge-note.md` §5.
