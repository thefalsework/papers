...
We define a *distinction structure* on a category as an idempotent endofunctor with unit, with monadic idempotency as a categorical rendering of the calling axiom of Spencer-Brown's *Laws of Form* (Spencer-Brown 1969). In an elementary topos equipped with a non-trivial distinction structure, we prove that the morphism space partitions into exactly four pairwise-disjoint structural classes, characterized by Heyting conditions on the morphism's image relative to the kernel image (the image of the unit). The four classes are mutually exclusive and jointly exhaustive over morphisms with non-trivial image. The proof reduces to standard Heyting algebra identities and a case analysis on lattice position. The fourth class — the *closure-residue* between a subobject and its double-negation — exists specifically as a feature of non-Boolean intuitionistic structure and collapses in Boolean settings. The construction is formalized in Lean 4 against Mathlib and kernel-checked; the underlying Heyting algebra instance on subobjects of an elementary topos was submitted to Mathlib as [PR #39618](https://github.com/leanprover-community/mathlib4/pull/39618) (closed unmerged under the project's new-contributor AI-use policy; the instance remains kernel-checked in this development).
...
The construction is implemented in Lean 4 against Mathlib4 (Mathlib Community 2026) and verified by the kernel. The underlying Heyting algebra instance on Sub(X) for elementary topoi — required by the partition theorem and missing from Mathlib's standard development — was submitted upstream as [PR #39618](https://github.com/leanprover-community/mathlib4/pull/39618) (closed unmerged on 2026-05-28 under Mathlib's new-contributor AI-use policy; it remains kernel-checked in this development) and follows the Mac Lane–Moerdijk equalizer-residual construction (Mac Lane and Moerdijk 1992, IV.6 Proposition 2).
...
The functor D is the *distinction operation*; η is the *unit* of the distinction; ι is the *idempotency*. The coherence condition relates the two natural morphisms from D(X) to D(X) and reflects the calling axiom of Spencer-Brown (1969): applying the distinction operation to an already-distinguished object is structurally equivalent to the original distinction. Monadic idempotency D ⋙ D ≅ D, together with the coherence condition, is one categorical rendering of Spencer-Brown's calling axiom (⟨ ⟩⟨ ⟩ = ⟨ ⟩, with the juxtaposition of marks translated to sequential functor composition). Other categorical readings of the calling axiom are possible (see Kauffman 1987 for an alternative recursive-form treatment); the framework's specific rendering is one option in that space, chosen here for its alignment with monad theory and reflective subcategories (Remark 5.5).
...
A *necessary condition on the witness object* is now kernel-checked and worth recording, because it constrains where to look. Since the kernel image is a_Y := Im(η_Y), and Im(η_Y) = ⊤ whenever η_Y is an isomorphism, Refusal (img ≤ a_Yᶜ = ⊥) is empty at every Y where the reflection unit is invertible — that is, at every j-sheaf, and in particular at the **terminal object**, which is a sheaf for every topology. So a four-cell witness can only occur at a Y that is *not* a j-sheaf (equivalently, an object the reflection actually moves). This generalizes the Sierpinski Candidate-E failure mode (sheafification has invertible unit on sheaves) into a topos-independent constraint. The Lean form is `kernelImage_eq_top_of_isIso_unit`; its specialization to the terminal is the reason the otherwise-natural escalation to a presheaf topos on a small poset — e.g. Set^{Pᵒᵖ} for P the three join-irreducibles of the subgroup lattice of ℤ/12, whose terminal subobject lattice Sub(1) is the kernel-checked six-element non-Boolean Heyting algebra Div12 ([`music-anchor/mazzola-bridge-note.md`](music-anchor/mazzola-bridge-note.md); `subobjectTerminalEquivDiv12`, `kernelImage_eq_top_of_isIso_unit`) — does *not* yield a sheafification witness *at the terminal*: the witness, if one exists in such a topos, must sit at a non-terminal, non-sheaf object. That refinement of the search is the live Phase-1.2 question.
...
A companion module `FalseWork.Lattice.FourPositionLattice` isolates the Heyting-algebra core of Theorem 5.1 as `lattice_four_position_partition` — for any `HeytingAlgebra H` and any `a x : H` with `x ≠ ⊥`, exactly one of the four position predicates holds at `(a, x)`. The topos-level `four_position_partition` is Layer L applied to `H = Sub(D Y)` with `a = Im(η_Y)` and `x = Im(D.map f)`. A concrete inhabitation of Layer L on a music-derived Heyting algebra lives at `FalseWork.Lattice.Examples.Div12`: the divisor lattice of 12 (equivalently the subgroup lattice of `Z/12`, equivalently the lattice of transposition-symmetric pitch-class subsets) is built as an explicit 6-element non-Boolean Heyting algebra; the witness `music_anchor_witness` establishes all four cells are inhabited at the tritone kernel by musically-meaningful subgroups (Infrastructure = tritone, Refusal = augmented triad, Exploitation = diminished 7th, Distribution = whole-tone hexachord and chromatic). Both are kernel-checked against the standard three axioms; `tritone_non_regular` is decidable and uses no axioms. The companion document [`music-anchor/feasibility.md`](music-anchor/feasibility.md) §12 records the layered (lattice / topos / distinction structure) status; Layer T (topos realization of this lattice via `Sh(L)` or the join-irreducible presheaf topos) and Layer D (a concrete idempotent monad delivering the kernel) are cited from general topos theory and deferred respectively.

A further companion module `FalseWork.Positions.CanonizationClosure` formalizes the framework's canonization apparatus in two related conditional theorems. `recursive_partition` is the *closure layer*: given a canonization-closure witness (data of an idempotent monad with iso multiplication) for a morphism `f`, the four-position partition theorem applies to the induced distinction structure on the ambient topos, producing a secondary four-cell classification of every morphism relative to the canonization figure. `canonization_separation` is the *generator refinement*: when the canonization data additionally certifies that the codomain `Y` of `f` is a separator (in the standard categorical sense — maps out of `Y` distinguish parallel morphisms), parallel morphisms in `C` agreeing under all `Y`-mediated comparisons are equal. Together the two theorems articulate the framework's claim that a Commitment-yes morphism organizes a structural territory (closure) within which it acts as a measuring instrument (generator). Both are kernel-checked against the standard three axioms; the substantive open question is the closure-side predicate identifying *which* idempotent monads count as canonization closures of a given morphism (three candidates surveyed in the companion document [`closure-canonization.md`](closure-canonization.md), selection deferred pending future work), while the generator-side separator condition is mathematically standard and concretely formalizable. A first kernel-checked concrete `CanonizationGenerator` witness (identity monad on `Discrete PUnit`) lives at `FalseWork.Positions.Examples.DiscretePUnit.canonizationGenerator`, establishing type-level inhabitation; richer instantiations remain open. A conditional cross-layer alignment definition `CanonizationGenerator.ofCommitmentYes` records the framework's architectural claim that a Commitment-yes morphism with a separator codomain admits a canonization-generator witness; the definition is kernel-checked but its proof does not yet use the commitment hypothesis because `IsCommitmentYes` is currently a schema-level placeholder. See [`closure-canonization.md`](closure-canonization.md) §§8a–8b for the full discussion.

The Heyting algebra structure on Sub(X) for elementary topoi — required by the partition theorem but not present in Mathlib at the time of this work — was submitted upstream as Pull Request [#39618](https://github.com/leanprover-community/mathlib4/pull/39618) to mathlib4 (Brink 2026, PR) and closed unmerged on 2026-05-28 under the project's new-contributor AI-use policy; the instance remains kernel-checked in this development. The PR implements the Mac Lane–Moerdijk equalizer-residual construction (Mac Lane and Moerdijk 1992, IV.6 Proposition 2): implication is defined as `Subobject.mk (equalizer.ι (χ (P ⊓ Q).arrow) (χ P.arrow))`, and the Galois connection R ≤ P ⇒ Q ↔ R ⊓ P ≤ Q is established through six private bridging lemmas. The hypothesis bundle required by the PR matches that of Theorem 5.1.
...
The theorem does not establish that any specific domain instantiates the structural condition. Whether music, cinema, painting, literature, software, physics, or other domains have the categorical structure the theorem requires is empirical work that the framework pursues separately (Brink 2026, Paper 3 on music; Paper 5 on the Pythagorean comma specifically). Two anchor scoping documents accompany this preprint: [`music-anchor/feasibility.md`](music-anchor/feasibility.md) records the music-side layered (lattice / topos / distinction structure) status, with the lattice layer kernel-checked in Lean (`FalseWork.Lattice.Examples.Div12.music_anchor_witness`); [`physics-anchor/feasibility.md`](physics-anchor/feasibility.md) scopes the physics-side target via Bohrification / topos quantum mechanics (Heunen-Landsman-Spitters 2009, Döring-Isham 2007), with no Lean layer kernel-checked in this round and a Wolfram Route-B exploration of small finite physics-interpretable lattices on record. Both documents are exploratory by design and the framework does not claim either anchor closes the empirical question of domain instantiation.
...
- Brink, C. (2026, PR). *Heyting algebra structure on Subobject in elementary topoi*. Mathlib4 Pull Request #39618 (opened 2026-05-20, closed unmerged 2026-05-28 under the project's new-contributor AI-use policy). [github.com/leanprover-community/mathlib4/pull/39618](https://github.com/leanprover-community/mathlib4/pull/39618).
