...
- that the §4.6-§4.7 v3 cell-cardinality results constitute a *categorical* witness to quantum non-classicality. They establish *quantitative sensitivity* of the partition machinery to non-commutativity on tight kernels, but the cell-non-emptiness criterion was uniformly negative in the M_2(C) ⊕ C discretisation. The categorical (sufficient-condition) question requires either a KS-bearing algebra (v4 path on M_3(C)) or a non-Heyting-derived kernel construction (research scope for a successor memo).

The concrete commitments this round are the Wolfram exploration scripts (`v1.wl`, `v2.wl`, `v2-p3.wl`, `v3.wl`), the structural core entry (`heunen-landsman-spitters-2009.wl`), the v3 scope memo (`v3-scope.md`), and this memo's scoping. The rest is scoping with explicit deferral.
