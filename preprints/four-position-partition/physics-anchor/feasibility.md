...
- that the §4.8-§4.9 v4 result constitutes the KS-induced structural break. The 4-MASA `M_3(C)` configuration is *not* KS-blocking; the structural-break signal is negative there. v4's positive finding is the *non-regularity of `δ(P)` itself* (Exploitation = 128), which is a stronger sensitivity finding than v3's but still a property of the chosen MASA shape under poset-iso comparison, not yet a categorical KS witness. The categorical witness needs a KS-blocking configuration (the v5 Penrose-40 / Peres-33 path).

The concrete commitments this round are the Wolfram exploration scripts (`v1.wl`, `v2.wl`, `v2-p3.wl`, `v3.wl`, `v4.wl`), the structural core entry (`heunen-landsman-spitters-2009.wl`), the v3 and v4 scope memos (`v3-scope.md`, `v4-scope.md`), and this memo's scoping. The rest is scoping with explicit deferral.
