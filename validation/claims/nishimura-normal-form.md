...
**Status:** OPEN — **CLOSURE COMPLETE (2026-06-17):** the ladder set `IsLadderValue g` is now kernel-checked closed under **all four** Heyting operations — complement, `⊔`, `⊓`, and now `⇨` (`isLadderValue_himp`). The subalgebra generated by `g` is exactly the Rieger–Nishimura ladder. All `sorry`-free, audited `[propext, Quot.sound]` (no `Classical.choice`). Remaining: wire this closure into `nishimura_kernel_unique` (drop [C]) and the `Div12` subalgebra upgrade (Phases 3–4 plumbing).
...
- Phases 3–4 plumbing: feed `isLadderValue_himp`/`_sup`/`_inf`/`_compl` into `nishimura_kernel_unique` to discharge [C]; upgrade `div12OrderEmbedding` to a subalgebra embedding (Citkin Prop. 3.1 in full).
