...
**Status:** OPEN — **[C] DISCHARGED (2026-06-17):** the ladder set `IsLadderValue g` is kernel-checked closed under **all four** Heyting operations — complement, `⊔`, `⊓`, `⇨` — and from this closure `generatedBy_isLadderValue` proves the **Nishimura enumeration** (every element of the subalgebra generated by `g` is a ladder value), formerly the classical citation **[C]**. The [C]-free kernel-uniqueness law `nishimura_kernel_unique_of_generated` follows. All `sorry`-free, audited `[propext, Quot.sound]` (no `Classical.choice`). Remaining: the `Div12` subalgebra-embedding upgrade (Citkin Prop. 3.1, Phase 4).
...
- **Implication table + full closure landed (2026-06-17), `Examples/NishimuraNormalForm.lean`** — all `sorry`-free, audited `[propext, Quot.sound]`:
  - **Terminal cell families:** `nishimuraTerm_himp_eq_top_of_le` (comparable ⟹ `⊤`); `xₘ ⇨ x₀ = xₘᶜ` (complement); `nishimuraTerm_himp_one` (`xₘ ⇨ x₁ = x₁`, `m ≥ 2`); `nishimuraTerm_himp_succ_odd` (`xₘ ⇨ xₘ₊₁ = xₘ₊₂`, `m` odd); the `n = m−1` diagonal `x_{2k+1} ⇨ x_{2k} = x_{2k+3}` (`nishimuraTerm_odd_eq_himp`).
  - **The decisive simplification — odd targets are terminal:** `nishimuraTerm_himp_eq_self_odd` proves `xₘ ⇨ x_{2i+3} = x_{2i+3}` whenever `x_{2i+1} ≤ xₘ` (currying collapses the antecedent meet `xₘ ⊓ x_{2i+1} = x_{2i+1}`, leaving the odd recursion). **Every** incomparable antecedent of an odd target satisfies this (it is `2i+1`, or `≥ 2i+4`), so odd targets need no recursion at all. This is what makes the induction close: the original worry — that the off-diagonal result *climbs* and the strong induction might carry a forward reference — dissolves, because the only climbing recursion (even targets) is one-dimensional in the target with a comparability ceiling.
  - **Structural reductions:** `isLadderValue_himp_even` (even target `x_{2k}` via `x_{2k} = x_{2k+1} ⊓ x_{2k+2}` and `⇨` distributing over `⊓`); even-antecedent join split `nishimuraTerm_even_eq_join` (`x_{2i+4} = x_{2i+1} ⊔ x_{2i+2}`) with `(a ⊔ b) ⇨ c = (a ⇨ c) ⊓ (b ⇨ c)`.
  - **Final assembly** `isLadderValue_himp_terms` / `isLadderValue_himp`: strong induction along the measure `μ(m,n) = 4·m − n` (fuel-bounded). Even target / even antecedent `m > 2k`: the join split drops `μ` (two strictly smaller antecedents). Even target / odd antecedent `m ≥ 2k+3`: the target climbs `2k ↦ 2k+2`, dropping `μ` by 2, with the odd half a terminal cell. All other cells are terminal. **Termination verified by the build, not by paper.**
- **[C] discharged (2026-06-17), `Examples/NishimuraNormalForm.lean`** — all `sorry`-free, audited `[propext, Quot.sound]`:
  - `GeneratedBy g` — inductive capture of the Heyting subalgebra generated by `g` (smallest set containing `g`, `⊤`, `⊥`, closed under `⊔`, `⊓`, `⇨`, `ᶜ`).
  - `generatedBy_isLadderValue` — **Nishimura's one-variable normal form**, the former [C]: `GeneratedBy g x ⟹ IsLadderValue g x`, by structural induction on the subalgebra using the four closure theorems (`isLadderValue_sup`/`_inf`/`_himp`/`_compl_of`). This is exactly "one-generated ⟹ every element is a term value", now kernel-checked rather than cited.
  - `nishimura_kernel_unique_of_generated` — kernel uniqueness with the cited enumeration hypothesis `hgen` replaced by the genuine one-generated property `∀ y, GeneratedBy g y`; discharges `hgen` via `generatedBy_isLadderValue`. The central weld no longer depends on [C]. The original conditional `nishimura_kernel_unique` (`NishimuraKernelLaw.lean`) is retained as the characterization the discharge feeds; its "What is NOT claimed" note now points to this discharge.
- **Phase 4 concrete witness (b) landed 2026-06-17** — `Examples.Div12.tritone_generates` (the tritone generates the music lattice as a Heyting subalgebra) and `kernel_unique_via_subalgebra` (third, [C]-free derivation of the unique kernel). See "Phase 4 scope" below; the abstract bundling (a) remains optional.

---

## Phase 4 scope — the subalgebra embedding (recorded 2026-06-17, before coding)

The standing one-line task was "upgrade `div12OrderEmbedding` to a subalgebra
embedding (Citkin Prop. 3.1 in full)". Scoping it surfaced a correctness flag that
has to be recorded *before* any code, because it is exactly the kind of
"looks-like-it-follows-but-doesn't" trap the project guards against.

### Finding 1 — the fully-abstract upgrade is FALSE (must not be stated)

`div12OrderEmbedding (hg : IsOrdinary g) : Div12 ↪o H` sends the six music-lattice
elements to `{⊥, g, gᶜ, gᶜᶜ, g ⊔ gᶜ, ⊤} = {x₀, x₂, x₁, x₃, x₄, ⊤}`. This is an
**order** embedding for *any* ordinary `g` in *any* `H`. It does **not** promote to
a Heyting-subalgebra embedding in general, because the six-element image is not
closed under the Heyting operations in the free algebra `F(1)`:

- `gᶜᶜ ⇨ g = x₃ ⇨ x₂ = x₅` by `nishimuraTerm_himp_diagonal` (n = 0) — and `x₅` is a
  genuinely distinct rung in `F(1)`, outside the six-set. (`gᶜ ⊔ gᶜᶜ` escapes too.)

So joins/meets/implications computed *in `H`* of image elements can land on rungs
above index 4. The subalgebra statement is true only when those higher rungs
**collapse** into the six-set — i.e. when `H` is one-generated by `g` with the
ladder truncating at the six-element level (equivalently `|⟨g⟩| = 6`, equivalently
`H ≅ Z₆`). A closure/collapse hypothesis is therefore mandatory; stating the
upgrade unconditionally would be a false [K].

### Finding 2 — the *correct* abstract content is already proved

Citkin Prop. 3.1's real content — "the subalgebra generated by `g` **is** the
Rieger–Nishimura ladder" — is exactly `generatedBy_isLadderValue` (landed today).
What is missing is only **bundling**: packaging the ladder-value set as a Mathlib
`Subalgebra`/`HeytingHom` object, plus, for the finite case, identifying the
collapsed ladder with the abstract `Z₆`.

### Finding 3 — the concrete "music IS this logic object" facts already exist

For the music lattice `Div12 = Z₆`, the Heyting-level identification is already in
the tree, just not bundled into one statement:

- `Div12.nishimura_generated` — `Div12` = the ladder values of the tritone (`decide`).
- `div12OrderIsoChains` with `toChains_himp` / `toChains_compl` (`WhyTwelve.lean`) —
  `Div12 ≃o C₃ × C₂` **preserving `⇨` and `ᶜ`**, i.e. already a Heyting iso.
- `ladderEmbed Div12.two` is the identity on `Div12` (the six images are the six
  elements), so the embedding is trivially onto at cardinality 6.

### Recommended Phase 4 (correctly scoped — *not* weeks; the hard math is done)

Two honest, independent deliverables:

- **(b) Concrete — LANDED 2026-06-17, `Examples/NishimuraNormalForm.lean`**, all
  `sorry`-free, audited (`tritone_generates` `[propext]`, `kernel_unique_via_subalgebra`
  `[propext, Quot.sound]`). Rather than a `Div12 ≃ Z₆` iso (which `div12OrderIsoChains`
  already gives for free, an order-iso between Heyting algebras being automatically
  Heyting), the honest headline is stated via the new generation machinery:
  - `Examples.Div12.tritone_generates : ∀ y : Div12, GeneratedBy Div12.two y` — the
    tritone generates the *whole* music lattice under the Heyting operations
    (`⊥ = two ⊓ twoᶜ`, `twoᶜ`, `twoᶜᶜ`, `two ⊔ twoᶜ`, `⊤`, each exhibited explicitly).
    "Music IS the one-generated Heyting algebra on the tritone", kernel-checked — the
    honest concrete form of Citkin Prop. 3.1, with **no hidden closure hypothesis**
    (at cardinality 6 the ladder has already collapsed, so Finding 1 does not bite).
  - `Examples.Div12.kernel_unique_via_subalgebra` — a *third* independent derivation of
    the unique kernel (the tritone), this time through the [C]-free
    `nishimura_kernel_unique_of_generated`. Agrees with the exhaustive `decide`
    (`Div12.kernel_unique`) and the enumeration weld (`Div12.kernel_unique_via_law`).
- **(a) Abstract bundling.** Promote `ladderEmbed` to a Heyting embedding under an
  explicit collapse hypothesis (cleanest: `H` one-generated by ordinary `g` with
  `gᶜᶜ ⇨ g ∈` the six-set, which forces the rest). Operation-preservation then
  reduces to the RN identities now in hand. Optional; (b) carries the music claim.

**Correctness note for the prose elsewhere in the corpus:** anywhere the
order-embedding is described as upgrading to a subalgebra "in full" for arbitrary
ordinary `g` should be softened to "for the one-generated / collapsed case", per
Finding 1. The `LadderCore.lean` docstring on `div12OrderEmbedding` already says
"order-embedding only — the Heyting-subalgebra strengthening is Citkin Prop. 3.1,
not claimed here", which is correct and should stay.
