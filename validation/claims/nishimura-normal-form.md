...
- Remaining Phase 4: the subalgebra-embedding statement. **Scoped 2026-06-17 — the naive phrasing is wrong; see "Phase 4 scope" below.**
...
- **(b) Concrete, recommended first — the headline.** A single bundled
  `Div12 ≃ Z₆` Heyting isomorphism (or a `HeytingHom` from the abstract six-element
  RN truncation onto `Div12` sending generator ↦ tritone). Both sides finite and
  decidable, so operation-preservation is `decide`. This is the literal
  "music IS `Z₆`" statement at the Heyting level.
