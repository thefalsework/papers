...
- **What's open:** (i) **Nishimura normal form — CLOSED (2026-06-17).** Every element of the subalgebra generated by `g` is a Nishimura term value in `g`, proved by closing the RN meet/join/implication tables as derived identities (`isLadderValue_sup`/`_inf`/`_himp`/`_compl`; `Examples/NishimuraNormalForm.lean`, `sorry`-free, axioms `propext, Quot.sound`). This **discharged the [C]** on the all-n kernel law: `generatedBy_isLadderValue` proves *one-generated ⟹ every element is a term value* (the former citation), and `nishimura_kernel_unique_of_generated` is the [C]-free law — the math anchor is now unconditional. On Citkin Prop. 3.1: the *fully-abstract* subalgebra upgrade of the order-embedding is **false** (scoped — the six-element image is not `⇨`-closed in `F(1)`, `gᶜᶜ ⇨ g = x₅` escapes); the honest concrete form is done (`Div12.tritone_generates` — the tritone generates the whole music lattice as a Heyting subalgebra; `Div12.kernel_unique_via_subalgebra` is a third, [C]-free derivation of the unique kernel). Only an optional bundled `HeytingHom`/`Subalgebra` wrapper (no new math) remains. Full record at [`nishimura-normal-form.md`](claims/nishimura-normal-form.md). (ii) **V(Z₆) primitiveness** — is the si-logic of the music lattice hereditarily structurally complete? Route written in the claim file (Citkin's criterion + Jónsson + cardinality); blocked only on reading the Hasse diagrams of prohibited algebras `P3–P5` from arXiv:2512.05633 Fig. 1 (figures, not text). A finite check once read.
- **Status:** (i) **CLOSED** — normal form kernel-checked, [C] discharged, weld unconditional; optional bundling only; (ii) blocked on figure access, then mechanical
...
- **Platform mirror:** the candidate is registered as `the-threshold` (status: `proposed`) on the FalseWork platform — kernel specification, five-territory topology, and pathway structure are seeded in the platform repo at `db/0167_threshold_kernel_genai.sql`.
