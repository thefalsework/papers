...
- `12-observer-cost.mjs` / `observer-cost.json` — registered 2026-09-24 (header frozen at `ca23907` before any code), run same day. **K12 did not fire: the relative falsework bound rank-orders head survival across observers** — pooled Spearman −0.908 over 16 cells, per-corpus −0.93/−0.93/−0.80 (disclosed scope: effective n ~3 corpora; licenses comparative use on a given graph, not a law). Both substantive registered guesses were wrong, informatively: aggregate near-exactness of the bound is observer-type-specific (partitions 1.02–1.16× on debian/crates, truncation blur up to 1.80×, and the non-dense observer inflates empty cones at bound 2 vs cost 1 — the per-cone equality is a nonempty-cone statement); and random size-matched partitions beat natural ones on head survival in 4 of 5 pairs — semantic classes are exactly what cones touch, so semantic blurs cost more phantom count at equal granularity, and the bound called the head-survival ordering in all five pairs. `mass_conserved_iff_dense` measured on both branches: the non-dense observer minted exactly one unit per inflated empty cone (9,164 / 22,960 / 1,106 across the three corpora).
