...
- `12-observer-cost.mjs` — **registered 2026-09-24, not yet run** (header committed before code, per program norms): does the relative falsework bound rank-order head survival across an observer family (natural partitions, size-matched random nulls, a truncation blur, a non-dense nucleus) on three corpora? Kill K12 frozen in the header; results will appear in a dated postscript.
